...
const Illustrasyon = () => {
  ...
  const fetchPortfolyo = async (tur) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/portfolio/illustrasyon`);
  ...
  return (
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 gap-x-9 px-10 sm:px-20 md:px-72 justify-items-center items-center">
      {portfolyolar.filter((item) => item.turler === "İllüstrasyon").slice(0, visibleCount).map((item, index) => (
